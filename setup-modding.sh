...
# =================================================================================
#               Maww-Toolkit v7.2 - Edisi Final: Simple & Stabil
#                      Powered by Maww-Core Engine v1.2
# =================================================================================
...
print_header() {
    clear
    echo -e "${C_CYAN}================================================================${C_NC}"
    echo -e "${C_CYAN}# ${C_NC}${C_YELLOW}Maww-Toolkit $TOOLKIT_VERSION: Simple & Stabil${C_NC}"
    echo -e "${C_CYAN}# ${C_NC}Manajemen Tool Modding & Analisis Aplikasi"
    echo -e "${C_CYAN}================================================================${C_NC}"
    echo
}

# Fungsi baru untuk mendapatkan versi terbaru GitHub tanpa caching
...
print_status_line() {
    ...
    # Format lebih rapi dan padat (kurangi padding)
...
manage_github_tool() {
    ...
    echo -e "\n [${C_YELLOW}S${C_NC}] Instal ${C_YELLOW}Stabil${C_NC} ($RECOMMENDED_VERSION) | [${C_BLUE}L${C_NC}] Instal ${C_BLUE}Terbaru${C_NC} ($LATEST_VERSION) | [${C_RED}H${C_NC}] Hapus | [${C_CYAN}B${C_NC}] Batal"
    ...
    if [ -z "$VERSION_TO_INSTALL" ] || [[ "$VERSION_TO_INSTALL" == "null" ]]; then echo -e "${C_RED}❌ Versi tidak valid atau tidak ditemukan!${C_NC}"; return; fi
    
    echo -e "${C_YELLOW}⏳ Memulai Unduhan $NAME v$VERSION_TO_INSTALL...${C_NC}"
    
    # URL Download harus dicari ulang karena LATEST_VERSION mungkin berbeda dengan versi stabil
    ...
    if [[ "$FILENAME_PATTERN" == "jadx" ]]; then
        ...
        echo -e "${C_YELLOW}✅ Unduhan Selesai. Mengekstrak dan Konfigurasi...${C_NC}"
        ...
        echo -e "${C_YELLOW}✅ Unduhan Selesai. Membuat Symlink Executable...${C_NC}"
...
# Konfigurasi Android SDK yang lebih jelas
func_configure_sdk_core(){ 
    ...
    if [ ! -d "$SDK_ROOT/cmdline-tools/latest" ]; then
        ...
        echo -e "${C_YELLOW} [2/4] Mengekstrak SDK Manager...${C_NC}"
    ...
    if [ -f "$SDKMANAGER" ]; then
        echo -e "${C_YELLOW} [3/4] Menginstal Paket SDK Wajib (platform-tools, build-tools 34.0.0)...${C_NC}"
        yes | "$SDKMANAGER" --licenses &>/dev/null # Setujui lisensi
        ("$SDKMANAGER" --install "$SDK_PACKAGES" &>/dev/null) & spinner
    fi

    echo -e "${C_YELLOW} [4/4] Mengkonfigurasi PATH di $PROFILE_FILE...${C_NC}"
    
    # Hapus konfigurasi SDK lama jika ada
...
manage_sdk() {
    ...
    case "$choice" in
        [Ii]) func_configure_sdk_core; echo -e "${C_GREEN}✅ Proses SDK Selesai!${C_NC}" ;;
        [Hh]) echo -e "${C_RED}Menghapus folder SDK total...${C_NC}"; rm -rf "$SDK_ROOT"; echo -e "${C_GREEN}✅ Dihapus.${C_NC}";;
        *) return ;;
    esac
}

# Fungsi Manajer Pkg dan Pip tetap sama (simple dan sudah rapi)
manage_pkg_tool() {
    ...
    if [[ "$choice" =~ ^[0-9]+$ ]] && [ "$choice" -ge 1 ] && [ "$choice" -le ${#TOOLS_DB[@]} ]; then
        ...
        case "$choice" in
            [Aa])
                ...
                (pkg install openjdk-17 -y) & spinner; echo -e "${C_GREEN}✅ Java OK.${C_NC}"

                echo -e "\n${C_CYAN}--- [2/5] Menginstal & Konfigurasi Android SDK ---${C_NC}"
                func_configure_sdk_core
                echo -e "${C_GREEN}✅ Android SDK OK.${C_NC}"
