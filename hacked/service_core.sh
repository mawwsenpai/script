...
func_setup() {
    ...
    echo "---------------------------------------------------------"
    echo -e "${C_YELLOW}⚠️  FILE KREDEKSIAL HARUS DITARUH DI PATH INI:${C_NC}"
    echo ">> $CREDS_FILE"
    echo "---------------------------------------------------------"
    ...
    if [ ! -f "$CREDS_FILE" ]; then
        echo -e "${C_RED}ERROR: File '$CREDS_FILENAME' tidak ditemukan di '$CREDS_FILE'. Setup dibatalkan. *Gajelas* sih kalau nggak ada file-nya. 🤪${C_NC}"
        exit 1
    fi
    ...
    if [ -f "$TOKEN_FILE" ]; then
        ...
        echo -e "\n${C_RED}SETUP GAGAL. Pastikan kamu memberikan izin di browser, sayangku! 🥺${C_NC}"
    fi
}
...
func_cleanup() {
    ...
    echo -e "${C_GREEN}✅ Konfigurasi (.conf, .log, .py, .json) telah dihapus.${C_NC}"
    echo -e "${C_YELLOW}⚠️  CATATAN: File '$CREDS_FILENAME' TIDAK dihapus dari '$HOME/storage/shared/Automatic/'.${C_NC}"
