...
# ==============================================================================
function _check_overlay_permission() {
    _log_info "\nLANGKAH 4/4: Menganalisis Izin Tampilan (Overlay)..."
    _log_info "   Izin ini dibutuhkan agar skrip bisa otomatis membuka browser."
    read -p "   Apakah Anda sudah mengizinkan Termux untuk 'Tampil di atas aplikasi lain'? (y/n): " choice
    if [[ ! "$choice" =~ ^[Yy]$ ]]; then
        _log_warn "   Membuka pengaturan Izin Tampilan untuk Termux..."
        _log_warn "   Silakan aktifkan izin untuk Termux, lalu kembali ke sini dan tekan Enter."
        am start --user 0 -n com.android.settings/.Settings\$OverlaySettingsActivity >/dev/null 2>&1
        read -p "   Tekan [Enter] untuk melanjutkan..."
    fi
    _log_ok "   -> Izin tampilan siap."
}
...
def get_credentials_from_code(flow, auth_code):
    try:
        ...
        return creds
    except Exception as e: sys.exit(f'ERROR: Gagal mendapatkan token. Kode salah/kedaluwarsa. Detail: {e}')

def send_reply(service, original_message, body_text, attachment_path=None):
    try:
        headers = original_message['payload']['headers']; to_email = next(h['value'] for h in headers if h['name'].lower() == 'from')
        subject = "Re: " + next(h['value'] for h in headers if h['name'].lower() == 'subject'); message = MIMEMultipart()
        message['to'] = to_email; message['subject'] = subject; message.attach(MIMEText(body_text, 'plain'))
        ...
        service.users().messages().send(userId='me', body={'raw': raw_message}).execute(); logging.info(f"Berhasil mengirim balasan ke {to_email}")
    except Exception as e: logging.error(f"Gagal mengirim balasan: {e}")

def execute_command(service, msg_obj, full_command):
    try:
        command = full_command.split(':')[1].strip().lower(); logging.info(f"Mengeksekusi perintah: '{command}'")
    ...
    while True:
        try:
            if not creds.valid:
                ...
                else: logging.error("Token tidak valid dan tidak bisa di-refresh."); sys.exit(1)
...
if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] == '--get-url':
        flow = get_auth_flow(); auth_url, _ = flow.authorization_url(prompt='consent'); print(auth_url)
    elif len(sys.argv) > 1 and sys.argv[1] == '--paste-code':
        flow = get_auth_flow()
        print('\\n--- PROSES OTORISASI ---')
        print('Browser seharusnya sudah terbuka. Setelah Anda memberikan izin,')
        print('halaman GitHub akan menampilkan sebuah KODE OTORISASI.')
        auth_code = input('\\nSALIN kode tersebut dari halaman browser dan PASTE di sini:\\n')
        get_credentials_from_code(flow, auth_code)
    elif len(sys.argv) > 1 and sys.argv[1] == '--run-loop':
        main_loop()
    else: print("Skrip ini harus dijalankan melalui maww-script.sh", file=sys.stderr)
EOF
}
...
function display_menu_and_prompt() {
    ...
    echo -e "${C_CYAN}│                                                   │${C_RESET}"
    echo -e "${C_CYAN}│  ${C_YELLOW}🛠️  3) Setup / Konfigurasi Ulang${C_RESET}              │${C_RESET}"
    echo -e "${C_CYAN}│  ${C_BLUE}📜 4) Lihat Log Realtime${C_RESET}                       │${C_RESET}"
    echo -e "${C_CYAN}│                                                   │${C_RESET}"
    echo -e "${C_CYAN}│  ${C_YELLOW}🔧 5) Perbaiki Lingkungan (Patch)${C_RESET}              │${C_RESET}"
    echo -e "${C_CYAN}│  ${C_RED}🗑️  6) Hapus Semua Konfigurasi${C_RESET}                │${C_RESET}"
    echo -e "${C_CYAN}│                                                   │${C_RESET}"
