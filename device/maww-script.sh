#!/usr/bin/env bash
# ==============================================================================
#                 MAWW SCRIPT V22 - PROFESSIONAL & SECURE
# ==============================================================================
# Deskripsi:
#   Versi final yang paling aman, stabil, dan dipoles. Menggunakan arsitektur
#   Web App -> Google Apps Script -> index.html yang terbukti andal.
#   Script ini TIDAK akan pernah mengekspos Client Secret di sisi klien.
#
...
function _log_header() { echo -e "${C_PURPLE}╭─ $@ ─${C_RESET}"; }
function _log_footer() { echo -e "${C_PURPLE}╰──────────────────────────────────────────────────╯${C_RESET}"; }
...
auth_code=sys.argv[1];creds_file="$G_CREDS_FILE";token_file="$G_TOKEN_FILE";scopes=['https://www.googleapis.com/auth/gmail.modify'];redirect_uri="$APPS_SCRIPT_URL"
...
except Exception as e:print(f"ERROR: Gagal menukar kode. Pastikan URL Apps Script benar. Detail: {e}",file=sys.stderr);sys.exit(1)
EOF
cat << EOF > "$PY_LISTENER"
...
function setup() {
    ...
    _log_header "Memulai Proses Setup (Jalur Aman: Web App)"
    rm -f "$G_TOKEN_FILE"
    if [ ! -f "$CONFIG_DEVICE" ]; then
        ...
        read -p "   - Paste URL Web App Google Apps Script Anda: " apps_script_url
        echo "MY_EMAIL=\"$email_input\"" > "$CONFIG_DEVICE"; echo "CMD_SUBJECT=\"$subject_input\"" >> "$CONFIG_DEVICE"; echo "APPS_SCRIPT_URL=\"$apps_script_url\"" >> "$CONFIG_DEVICE"
        _log_ok "   -> Konfigurasi berhasil disimpan."
    fi
    source "$CONFIG_DEVICE"
    if [ -z "$APPS_SCRIPT_URL" ]; then
        _log_error "GAGAL: URL Apps Script tidak ditemukan di '$CONFIG_DEVICE'."
        _log_error "Jalankan opsi '6) Hapus Konfigurasi' lalu ulangi setup."
        _log_footer; return
    fi
    _log_info "Memeriksa '$G_CREDS_FILE'..."; if [ ! -f "$G_CREDS_FILE" ]; then
        if ! cp "$DOWNLOAD_DIR/$G_CREDS_FILE" .; then _log_error "GAGAL: '$G_CREDS_FILE' tidak ditemukan."; _log_footer; return; fi
        _log_ok "   -> Berhasil disalin dari Download."; fi
    if [ ! -f "$CONFIG_FILE" ]; then
        _log_warn "   -> '$CONFIG_FILE' tidak ada. Membuat otomatis..."
        local client_id_from_creds=$(grep -o '"client_id": *"[^"]*"' "$G_CREDS_FILE" | grep -o '"[^"]*"$' | tr -d '"')
        if [ -z "$client_id_from_creds" ]; then _log_error "GAGAL: Tidak bisa membaca client_id dari '$G_CREDS_FILE'."; _log_footer; return; fi
        echo "{\"CLIENT_ID\": \"${client_id_from_creds}\"}" > "$CONFIG_FILE"
        _log_ok "   -> Berhasil dibuat."; fi
    _log_info "Membuat helper script..."; _generate_py_scripts
    local client_id=$(grep -o '"CLIENT_ID": *"[^"]*"' "$CONFIG_FILE" | grep -o '"[^"]*"$' | tr -d '"')
    if [ -z "$client_id" ]; then _log_error "GAGAL: CLIENT_ID kosong di '$CONFIG_FILE'."; _log_footer; return; fi
    _log_ok "   -> Client ID berhasil dibaca."
    local scope="https://www.googleapis.com/auth/gmail.modify"
    local auth_url="https://accounts.google.com/o/oauth2/v2/auth?scope=${scope}&redirect_uri=${APPS_SCRIPT_URL}&response_type=code&client_id=${client_id}&access_type=offline&prompt=consent"
    _log_info "Membuka browser untuk otentikasi..."
    am start -a android.intent.action.VIEW -d "$auth_url"
    _log_warn "================= INSTRUKSI MANUAL ================="
    _log_warn "1. Selesaikan login di browser yang terbuka."
    _log_warn "2. Anda akan diarahkan ke halaman 'index.html' Anda."
    _log_warn "3. Salin kode yang ditampilkan di halaman tersebut."
    _log_warn "===================================================="
    while true; do
        read -p "   - Paste kode di sini (atau 'q' untuk keluar): " manual_code
        if [[ "$manual_code" == "q" ]]; then _log_info "Setup dibatalkan."; _log_footer; return; fi
        if python "$PY_HELPER_TOKEN" "$manual_code"; then break
        else _log_error "Kode salah atau tidak valid. Coba lagi."; fi
    done
    if [ -f "$G_TOKEN_FILE" ]; then _log_ok "🎉 SETUP SELESAI! Otorisasi berhasil."; else _log_error "SETUP GAGAL."; fi
    _log_footer
}

function start(){ clear;display_header;_log_header "Memulai Listener";_log_info "Mencoba memulai listener...";if [ ! -f "$CONFIG_DEVICE" ]||[ ! -f "$G_TOKEN_FILE" ];then _log_error "Konfigurasi/token tidak ditemukan. Jalankan 'Setup' (3) dulu.";_log_footer;return;fi;if [ -f "$PID_FILE" ]&&ps -p "$(cat "$PID_FILE")" >/dev/null;then _log_warn "Listener sudah berjalan.";_log_footer;return;fi;_generate_py_scripts;nohup python "$PY_LISTENER" >/dev/null 2>&1 & echo $! > "$PID_FILE";_log_ok "Listener dimulai (PID: $(cat "$PID_FILE")). Log di '$LOG_FILE'." ;_log_footer;}
function stop(){ clear;display_header;_log_header "Menghentikan Listener";_log_info "Mencoba menghentikan listener...";if [ ! -f "$PID_FILE" ];then _log_warn "Listener tidak sedang berjalan.";_log_footer;return;fi;local pid=$(cat "$PID_FILE");if ps -p "$pid" >/dev/null;then kill "$pid";rm -f "$PID_FILE";_log_ok "Listener (PID: $pid) telah dihentikan.";else _log_warn "Proses (PID: $pid) tidak ditemukan.";rm -f "$PID_FILE";fi;_log_footer;}
function logs(){ clear;display_header;_log_header "Melihat Log";if [ ! -f "$LOG_FILE" ];then _log_warn "File log belum ada.";_log_footer;return;fi;_log_info "Menampilkan log (tekan Ctrl+C untuk keluar)...";tail -f "$LOG_FILE" ;}
function cleanup(){ clear;display_header;_log_header "Pembersihan Total";_log_warn "Ini akan menghapus SEMUA file terkait skrip ini.";read -p "   Anda yakin ingin melanjutkan? (y/n): " confirm;if [[ "$confirm" =~ ^[Yy]$ ]];then stop >/dev/null 2>&1||true;_log_info "Menghapus file...";rm -f "$CONFIG_DEVICE" "$G_TOKEN_FILE" "$G_CREDS_FILE" "$CONFIG_FILE" "$PY_HELPER_TOKEN" "$PY_LISTENER" ".patch_installed" "$LOG_FILE" "$PID_FILE";_log_ok "Pembersihan selesai.";else _log_info "Pembersihan dibatalkan.";fi;_log_footer;}
function run_patcher(){ set -e;clear;display_header;_log_header "Persiapan Lingkungan";readonly PKS=("python" "termux-api" "coreutils");readonly PYR=("google-api-python-client" "google-auth-httplib2" "google-auth-oauthlib");_log_info "Langkah 1/3: Memeriksa paket sistem...";pkg update -y >/dev/null 2>&1;for p in "${PKS[@]}";do if ! dpkg -s "$p">/dev/null 2>&1;then _log_warn "   -> Instal '$p'...";pkg install -y "$p";fi;done;_log_ok "   -> Paket sistem siap.";_log_info "Langkah 2/3: Memeriksa library Python...";for r in "${PYR[@]}";do if ! pip show "$r">/dev/null 2>&1;then _log_warn "   -> Instal '$r'...";pip install --no-cache-dir "$r";fi;done;_log_ok "   -> Library Python siap.";_log_info "Langkah 3/3: Izin penyimpanan...";if [ ! -d "$HOME/storage/shared" ];then termux-setup-storage;_log_warn "   -> Izin diminta...";sleep 5;fi;_log_ok "   -> Izin penyimpanan siap.";echo;_log_ok "✅ LINGKUNGAN SUDAH SIAP! ✅";set +e;_log_footer;}

function display_header(){ echo -e "${C_PURPLE}";echo '  ╭──────────────────────────────────────────────────╮';echo -e "  │ ${C_BG_PURPLE}${C_WHITE}    ⓂⒶⓌⓌ - ⓈⒸⓇⒾⓅⓉ  v22 (Pro & Secure)    ${C_RESET}${C_PURPLE} │";echo '  ├──────────────────────────────────────────────────┤';local status_text;local status_color;if [ -f "$PID_FILE" ]&&ps -p "$(cat "$PID_FILE")" >/dev/null;then status_text="    A K T I F    ";status_color="$C_GREEN";pid_text="(PID: $(cat "$PID_FILE"))";else status_text="  TIDAK AKTIF  ";status_color="$C_RED";pid_text="";fi;echo -e "  │ ${C_WHITE}Status   :${C_RESET} ${status_color}${status_text}${C_RESET} ${C_YELLOW}${pid_text}${C_RESET}                     ${C_PURPLE}│";echo '  ╰──────────────────────────────────────────────────╯';echo -e "${C_RESET}" ;}
function display_menu(){ echo -e "${C_CYAN}  ╭─ Pilihan Menu ───────────────────────────────╮${C_RESET}";echo -e "${C_CYAN}  │                                              │${C_RESET}";echo -e "${C_CYAN}  │   ${C_GREEN}1) 🚀  Mulai Listener${C_RESET}                       │${C_RESET}";echo -e "${C_CYAN}  │   ${C_RED}2) 🛑  Hentikan Listener${C_RESET}                    │${C_RESET}";echo -e "${C_CYAN}  │                                              │${C_RESET}";echo -e "${C_CYAN}  │   ${C_YELLOW}3) ⚙️   Setup / Konfigurasi Ulang${C_RESET}          │${C_RESET}";echo -e "${C_CYAN}  │   ${C_PURPLE}4) 📜  Lihat Log Realtime${C_RESET}                 │${C_RESET}";echo -e "${C_CYAN}  │                                              │${C_RESET}";echo -e "${C_CYAN}  │   ${C_BLUE}5) 🔧  Perbaiki Lingkungan${C_RESET}                │${C_RESET}";echo -e "${C_CYAN}  │   ${C_RED}6) 🗑️   Hapus Semua Konfigurasi${C_RESET}            │${C_RESET}";echo -e "${C_CYAN}  │                                              │${C_RESET}";echo -e "${C_CYAN}  │   ${C_WHITE}7) 🚪  Keluar${C_RESET}                                 │${C_RESET}";echo -e "${C_CYAN}  │                                              │${C_RESET}";echo -e "${C_CYAN}  ╰──────────────────────────────────────────────╯${C_RESET}";echo -en "\n${C_BG_CYAN}${C_BLACK}  Pilih Opsi [1-7] ❯ ${C_RESET} ";read -n 1 choice;echo;case $choice in '1') start;;'2') stop;;'3') setup;;'4') logs;;'5') run_patcher; touch .patch_installed ;;'6') cleanup;;'7') echo -e "\n${C_PURPLE}Sampai jumpa lagi, Senpai!${C_RESET}";exit 0;;*) echo -e "\n${C_RED}Pilihan tidak valid.${C_RESET}";;esac;echo -e "\n${C_YELLOW}Tekan [Enter] untuk kembali...${C_RESET}";read -r ;}
function main(){ if [ ! -f ".patch_installed" ];then clear;display_header;_log_header "Selamat Datang!";_log_info "Ini eksekusi pertama. Skrip akan menyiapkan lingkungan.";_log_warn "Pastikan 'credentials.json' (tipe Web App) ada di folder Download.";read -p "   Tekan [Enter] untuk memulai persiapan...";run_patcher;_log_info "Membuat file penanda penyelesaian...";touch .patch_installed;if [ $? -eq 0 ];then _log_ok "   -> Penanda '.patch_installed' berhasil dibuat.";else _log_error "   -> KRITIS: Gagal membuat file penanda.";fi;read -p "   Tekan [Enter] untuk lanjut...";fi;while true;do clear;display_header;display_menu;done ;}
