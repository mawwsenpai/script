#!/usr/bin/env bash
# ==============================================================================
#                 MAWW SCRIPT V21 - THE FINAL TRUTH
# ==============================================================================
# Deskripsi:
#   Versi definitif dengan alur otentikasi yang benar secara fundamental.
#   Menggunakan kredensial "Web Application" dan library "Flow" yang sesuai,
#   dengan Google Apps Script sebagai jembatan redirect yang stabil.
#
...
# [PERBAIKAN FUNDAMENTAL] Menggunakan library 'Flow' yang sesuai untuk kredensial "Web App".
...
if len(sys.argv) < 2:
    print("Penggunaan: python handle_token.py <auth_code>", file=sys.stderr)
    sys.exit(1)
...
redirect_uri = "$APPS_SCRIPT_URL"

if not os.path.exists(creds_file):
    print(f"FATAL: File '{creds_file}' tidak ditemukan!", file=sys.stderr)
    sys.exit(1)
...
try:
    ...
    print(f"ERROR: Gagal menukar kode. Pastikan URL Apps Script sudah benar di config & Google Cloud. Detail: {e}", file=sys.stderr)
    sys.exit(1)
EOF

# Skrip listener utama (tidak ada perubahan signifikan).
cat << EOF > "$PY_LISTENER"
...
function setup() {
    ...
    _log_warn "Memulai proses Setup (Jalur Web App)..."
    rm -f "$G_TOKEN_FILE"
    ...
    _log_info "Memeriksa '$G_CREDS_FILE'..."; if [ ! -f "$G_CREDS_FILE" ]; then
        if ! cp "$DOWNLOAD_DIR/$G_CREDS_FILE" .; then _log_error "GAGAL: '$G_CREDS_FILE' tidak ditemukan."; return; fi
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        if [ -z "$client_id_from_creds" ]; then _log_error "GAGAL: Tidak bisa membaca client_id dari '$G_CREDS_FILE'."; return; fi
    ...
    if [ -z "$client_id" ]; then _log_error "GAGAL: CLIENT_ID kosong di '$CONFIG_FILE'."; return; fi
    ...
    _log_warn "1. Selesaikan login di browser."
    ...
    if [ -f "$G_TOKEN_FILE" ]; then _log_ok "🎉 SETUP SELESAI! Otorisasi berhasil."; else _log_error "SETUP GAGAL."; return; fi
}

# (Sisa skrip start, stop, logs, cleanup, run_patcher, menu, main... tidak berubah)
...
function logs(){ clear;display_header;if [ ! -f "$LOG_FILE" ];then _log_warn "File log belum ada.";return;fi;_log_info "Menampilkan log (Ctrl+C untuk keluar)...";echo -e "${C_PURPLE}╭──────────────────────────────────────────────────╮${C_RESET}";tail -f "$LOG_FILE" ;}
function cleanup(){ clear;display_header;_log_warn "PERINGATAN: Ini akan menghapus SEMUA file terkait skrip ini.";read -p "   Anda yakin ingin melanjutkan? (y/n): " confirm;if [[ "$confirm" =~ ^[Yy]$ ]];then stop >/dev/null 2>&1||true;_log_info "Menghapus file...";rm -f "$CONFIG_DEVICE" "$G_TOKEN_FILE" "$G_CREDS_FILE" "$CONFIG_FILE" "$PY_HELPER_TOKEN" "$PY_LISTENER" ".patch_installed" "$LOG_FILE" "$PID_FILE";_log_ok "Pembersihan selesai.";else _log_info "Pembersihan dibatalkan.";fi ;}
function run_patcher(){ set -e;clear;display_header;_log_info "Memulai Proses Persiapan Lingkungan...";readonly TERMUX_PACKAGES=("python" "termux-api" "coreutils");readonly PYTHON_REQUIREMENTS=("google-api-python-client" "google-auth-httplib2" "google-auth-oauthlib");_log_info "LANGKAH 1/3: Memeriksa paket sistem...";pkg update -y >/dev/null 2>&1;for pkg in "${TERMUX_PACKAGES[@]}";do if ! dpkg -s "$pkg" >/dev/null 2>&1;then _log_warn "   -> Menginstal '$pkg'...";pkg install -y "$pkg";fi;done;_log_ok "   -> Paket sistem sudah siap.";_log_info "LANGKAH 2/3: Memeriksa library Python...";for req in "${PYTHON_REQUIREMENTS[@]}";do if ! pip show "$req" >/dev/null 2>&1;then _log_warn "   -> Menginstal '$req'...";pip install --no-cache-dir "$req";fi;done;_log_ok "   -> Library Python sudah siap.";_log_info "LANGKAH 3/3: Mengonfigurasi izin penyimpanan...";if [ ! -d "$HOME/storage/shared" ];then termux-setup-storage;_log_warn "   -> Izin penyimpanan diminta...";sleep 5;fi;_log_ok "   -> Izin penyimpanan siap.";echo;_log_ok "✅ LINGKUNGAN SUDAH SIAP! ✅";set +e ;}
function display_header(){ echo -e "${C_PURPLE}";echo '  ╭──────────────────────────────────────────────────╮';echo -e "  │ ${C_BG_PURPLE}${C_WHITE}    ⓂⒶⓌⓌ - ⓈⒸⓇⒾⓅⓉ  v21 (The Final Truth)   ${C_RESET}${C_PURPLE} │";echo '  ├──────────────────────────────────────────────────┤';local status_text;local status_color;if [ -f "$PID_FILE" ]&&ps -p "$(cat "$PID_FILE")" >/dev/null;then status_text="    A K T I F    ";status_color="$C_GREEN";pid_text="(PID: $(cat "$PID_FILE"))";else status_text="  TIDAK AKTIF  ";status_color="$C_RED";pid_text="";fi;echo -e "  │ ${C_WHITE}Status   :${C_RESET} ${status_color}${text}${C_RESET} ${C_YELLOW}${pid_text}${C_RESET}                     ${C_PURPLE}│";echo '  ╰──────────────────────────────────────────────────╯';echo -e "${C_RESET}" ;}
