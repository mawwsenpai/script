...
try:
    ...
    print(f"ERROR: Gagal menukar kode dengan token. Pastikan kode valid. Detail: {e}", file=sys.stderr)
    sys.exit(1)
EOF
}
...
def send_reply(service, original_message, body_text, attachment_path=None):
    try:
        headers=original_message['payload']['headers']; to_email=next(h['value'] for h in headers if h['name'].lower()=='from'); subject="Re: "+next(h['value'] for h in headers if h['name'].lower()=='subject')
        message=MIMEMultipart(); message['to']=to_email; message['subject']=subject; message.attach(MIMEText(body_text,'plain'))
...
def execute_command(service, msg_obj, full_command):
    try:
        ...
        elif command=='exit-listener': reply_body="Perintah 'exit-listener' diterima. Listener akan berhenti."; send_reply(service,msg_obj,reply_body); logging.info("Listener dihentikan melalui perintah remote."); sys.exit(0)
        else: reply_body=f"Perintah '{command}' tidak dikenali."
        send_reply(service,msg_obj,reply_body,output_file)
    except Exception as e: logging.error(f"Error saat eksekusi: {e}"); send_reply(service,msg_obj,f"GAGAL: Terjadi error. Cek log di server.")
    finally:
...
function setup() {
    ...
    if [ ! -f "$G_CREDS_FILE" ]; then
        if [ -f "$DOWNLOAD_DIR/$G_CREDS_FILE" ]; then
            cp "$DOWNLOAD_DIR/$G_CREDS_FILE" .; _log_ok "   -> '$G_CREDS_FILE' disalin dari folder Download."
        else
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        _log_ok "   -> '$CONFIG_FILE' berhasil dibuat."
    fi
    ...
    if [ -f "auth_code.tmp" ]; then
        ...
        _log_warn "================= MODE MANUAL ANTI-GAGAL ================="
        _log_warn "Skrip akan terus meminta kode sampai berhasil."
        _log_warn "Buka URL di browser, login, lalu salin kodenya dari address bar."
        _log_warn "=========================================================="
        python -m webbrowser -t "$auth_url"
        while true; do
            read -p "   - Paste kode di sini (atau 'q' untuk keluar): " manual_code
            if [[ "$manual_code" == "q" ]]; then break; fi
    ...
    if [ -f "$G_TOKEN_FILE" ]; then _log_ok "🎉 SETUP SELESAI! Otorisasi berhasil."; else _log_error "SETUP DIBATALKAN atau GAGAL."; fi
    set +e
}
...
function stop() {
    ...
    if ps -p "$pid" > /dev/null; then kill "$pid"; rm -f "$PID_FILE"; _log_ok "Listener (PID: $pid) telah dihentikan."
    else _log_warn "Proses (PID: $pid) tidak ditemukan. Menghapus file PID."; rm -f "$PID_FILE"; fi
}
...
function logs() {
    ...
    echo -e "${C_PURPLE}╭──────────────────────────────────────────────────╮${C_RESET}"; tail -f "$LOG_FILE"
}

function cleanup() {
    clear; display_header
    _log_warn "PERINGATAN: Ini akan menghapus SEMUA file terkait skrip ini."
...
function display_menu() {
    ...
    case $choice in
        ...
        *) echo -e "\n${C_RED}Pilihan tidak valid. Masukkan angka dari 1 sampai 7.${C_RESET}";;
    esac
